...
export default function TaskDetailPanel() {
  ...
  useEffect(() => {
    if (selectedTask) {
      ...
      setEditPlatform(selectedTask.platform.name);
      setIsDirty(false);
    }
  }, [selectedTask?.id]);
  ...
  const currentStatus =
    STATUS_CONFIG[selectedTaskColumnId as keyof typeof STATUS_CONFIG];
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
              <div className="flex items-center gap-2">
                {currentStatus && (
                  <span
                    className={`text-[11px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1.5 ${currentStatus.color}`}
                  >
                    ...
                    {showMoveMenu && (
                      ...
                      >
                        {otherColumns.map((col) => {
                          const s = STATUS_CONFIG[col.id as keyof typeof STATUS_CONFIG];
                          return (
                            <button
                              key={col.id}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              {/* Author avatar (if exists) */}
              {selectedTask.author && (
                <div className="flex items-center gap-3">
                  <img
                    src={selectedTask.author.avatar}
                    className="w-10 h-10 rounded-full ring-2 ring-slate-100"
                    alt={selectedTask.author.name}
                  />
                  <div>
                ...
                {isDirty ? (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        onClick={handleSave}
                        className="h-9 px-5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm shadow-sm"
                      >
                        Save changes
                      </Button>
                    </div>
