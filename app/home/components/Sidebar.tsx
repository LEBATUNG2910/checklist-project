...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  ...
  const { columns, fetchColumns, initialized } = useTaskStore();
  ...
  useEffect(() => {
    setMounted(true);
    // Tự động fetch lại dữ liệu thật từ DB theo account hiện tại nếu chưa init
    if (!initialized) {
      fetchColumns();
    }
  }, [initialized, fetchColumns]);

  // Đếm số lượng task thật thuộc từng platform từ database của user hiện tại
  ...
  return (
    <div className="flex flex-col h-full bg-background transition-colors duration-300">
        ...
        <div className="px-4 py-4">
          ...
          <nav className="space-y-1">
            {MESSAGE_ITEMS.map((item) => {
              ...
              const isActive = activeMenu === key;
              // Chỉ lấy số đếm thật từ DB, loại bỏ hoàn toàn mock badge cố định
              const unread = getDynamicUnreadCount(item.label);
              return (
                <button key={item.id} onClick={() => handleMenuClick(key)}
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-1">
        {/* Nút Settings — dùng activeMenu thay vì Link */}
        <button
          onClick={() => handleMenuClick("settings" as MenuKey)}
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
            activeMenu === "settings"
              ? "bg-blue-600 text-white shadow-md shadow-blue-200 dark:shadow-none"
              : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50"
        ...
        {/* Dark mode toggle */}
        <div className="flex items-center justify-between px-3 py-2.5 text-slate-600 dark:text-slate-400 rounded-xl transition-colors">
          <div className="flex items-center gap-3">
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-slate-900/40 dark:bg-slate-900/80 backdrop-blur-sm z-30 md:hidden transition-colors duration-300"
            />
